...
for (const file of files) {
  ...
  if (!response.ok) throw new Error(`Failed to fetch Elite Poker backend file ${file}: ${response.status}`);
...
server = server.replace(/const PORT = process\.env\.PORT \|\| 3000;[\s\S]*$/, "export default server;\n");
